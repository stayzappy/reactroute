...
Write-Host " ReactRoute Protocol Builder (Dart2JS)"
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Inject Telemetry Loading Screen (ReactRoute Styling)
$loadingDiv = @"
<div id="loading">
<style>
...
.spinner { border: 4px solid rgba(255, 255, 255, 0.1); border-left-color: #8B5CF6; border-radius: 50%; width: 48px; height: 48px; animation: spin 1s linear infinite; margin: 0 auto 20px auto; }
@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
.loading-text { font-size: 18px; font-weight: bold; letter-spacing: 1px; color: #8B5CF6; }
</style>
<div style="text-align: center;">
    <div class="spinner" id="spinner"></div>
    <div class="loading-text" id="loading-title">INITIALIZING REACTROUTE...</div>
</div>
</div>
<script>
    function logBoot(msg, isError = false) {
        if (isError) {
            console.error('[ReactRoute Boot] ' + msg);
        } else {
            console.log('[ReactRoute Boot] ' + msg);
        }
    }
...
$buildLog = "<script>console.log('ReactRoute Build Version: $timestamp');</script>"
